...
figura <- ggplot(cascata, aes(persone, voce, fill = voce)) +
  ...
  labs(
    ...
    subtitle = sommario(paste0(
      "Scomposizione della variazione del numero di 15-24enni di Bagheria in ciascuno dei quattro stati della condizione professionale, fra il ", DA, " e il ", A,
      ", in PERSONE: quante ne toglie o ne aggiunge il restringersi della coorte, e quante il cambiamento del tasso. Le due sommano alla variazione netta. ",
      "È aritmetica, non un modello: non attribuisce cause al di là del denominatore.\n",
      "Chi cerca lavoro cala di ", migliaia(round(abs(netto("in_cerca")))),
      " persone e quasi tutto è cambiamento di tasso (", migliaia(round(tasso("in_cerca"))), ");\n",
      ...
      "Il nucleo che non cerca si è quasi solo rimpicciolito con la demografia."), LARGHEZZA),
    ...
    caption = didascalia_2b(
      lettura = paste0(
        ...
        "La scomposizione è shift-share sui due estremi del periodo: l'effetto popolazione è la variazione della coorte a tasso ", DA,
        " fermo, l'effetto tasso è la variazione del tasso applicata alla coorte ", A, ", e la somma è esatta, con controllo nel notebook. ",
        "Le variazioni in persone non sommano a zero fra i quattro pannelli, perché la coorte 15-24 si è ristretta (da ", migliaia(round(enne_coorte(DA))),
        " a ", migliaia(round(enne_coorte(A))), " residenti): la loro somma è esattamente l'effetto demografico complessivo, mentre a sommare a zero sono gli effetti di tasso. ",
