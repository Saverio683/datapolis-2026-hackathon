...
def _load_clean() -> tuple[pd.DataFrame, pd.DataFrame, pd.DataFrame, pd.DataFrame]:
    ...
    current = pd.read_csv(
        PROCESSED / "census_education_work_long.csv",
        dtype={"territorio": str, "condizione": str},
    )
    population = pd.read_csv(PROCESSED / "census_population_long.csv", dtype={"territorio": str})
    commuting = pd.read_csv(PROCESSED / "census_commuting_long.csv", dtype={"territorio": str})
...
def source_inventory(
    ...
    schools = pd.read_csv(PROCESSED / "technical_schools.csv", dtype=str)
    gtfs = pd.read_csv(PROCESSED / "palermo_gtfs_summary.csv")
    rows = [
        {
...
def run_analysis() -> dict[str, object]:
    ...
    tables = {
        ...
        "historical_bagheria.csv": historical,
        "historical_benchmarks_2011.csv": historical_benchmarks,
        "historical_change_1991_2011.csv": historical_change,
        "youth_states_2018_2024.csv": youth,
        "education_context_2018_2024.csv": education,
        "adult_transition_2018_2024.csv": adult,
        "youth_population_15_34.csv": population,
        "recent_analysis_panel.csv": panel,
        "gaps_vs_sicily.csv": gaps,
        "change_decomposition_2018_2024.csv": decomposition,
        "commuting_appendix.csv": commuting,
        "matched_peers_2011.csv": peers,
        "model_robustness_2011.csv": robustness,
        "kpi_dashboard.csv": dashboard,
        "finding_summary.csv": findings,
    }
