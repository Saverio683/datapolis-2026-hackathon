...
def historical_change_chart() -> Path:
    data = pd.read_csv(TABLES / "historical_change_1991_2011.csv")
    positive = {"I6", "I7", "L14"}
...
def transition_benchmark_chart() -> Path:
    data = pd.read_csv(TABLES / "historical_benchmarks_2011.csv")
...
def youth_composition_trend_chart() -> Path:
    data = pd.read_csv(TABLES / "youth_states_2018_2024.csv", dtype={"territorio": str})
...
def youth_benchmark_chart() -> Path:
    data = pd.read_csv(TABLES / "youth_states_2018_2024.csv")
...
def convergence_gap_chart() -> Path:
    data = pd.read_csv(TABLES / "gaps_vs_sicily.csv")
    specs = [
...
def decomposition_chart() -> Path:
    data = pd.read_csv(TABLES / "change_decomposition_2018_2024.csv")
...
def adult_trend_chart() -> Path:
    data = pd.read_csv(TABLES / "adult_transition_2018_2024.csv")
...
def adult_position_chart() -> Path:
    data = pd.read_csv(TABLES / "adult_transition_2018_2024.csv").dropna(subset=["quota_occupati_25_49"])
...
def population_chart() -> Path:
    data = pd.read_csv(TABLES / "youth_population_15_34.csv")
    colors = {"Bagheria": BLUE, "Palermo": PURPLE, "Sicilia": ORANGE, "Italia": GREEN}
...
def peer_chart() -> Path:
    data = pd.read_csv(TABLES / "matched_peers_2011.csv")
...
def model_robustness_chart() -> Path:
    data = pd.read_csv(TABLES / "model_robustness_2011.csv")
    labels = {
...
def mobility_appendix_chart() -> Path:
    data = pd.read_csv(TABLES / "commuting_appendix.csv").set_index("territorio_nome").reindex(["Bagheria", "Palermo", "Sicilia", "Italia"])
