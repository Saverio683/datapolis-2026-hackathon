...
class PipelineContractTests(unittest.TestCase):
    ...
    def setUpClass(cls) -> None:
        cls.quality = json.loads((PROCESSED / "edu_quality_report.json").read_text(encoding="utf-8"))
        cls.validation = json.loads(
            (DOCS_EDU / "validation_report.json").read_text(encoding="utf-8")
        )
        cls.summary = json.loads((TABLES / "edu_analysis_summary.json").read_text(encoding="utf-8"))
        cls.youth = pd.read_csv(TABLES / "edu_youth_states_2018_2024.csv", dtype={"territorio": str})
        cls.gaps = pd.read_csv(TABLES / "edu_gaps_vs_sicily.csv")
        cls.decomposition = pd.read_csv(TABLES / "edu_change_decomposition_2018_2024.csv")
    ...
    def test_peer_matching_does_not_use_outcomes(self) -> None:
        config = yaml.safe_load((ROOT / "pipeline" / "edu" / "config" / "analysis.yml").read_text(encoding="utf-8"))
    ...
    def test_models_are_appendix_only_and_exclude_bagheria(self) -> None:
        models = pd.read_csv(TABLES / "edu_model_robustness_2011.csv")
